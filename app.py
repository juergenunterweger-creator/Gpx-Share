...
with st.expander("ℹ️ Über GPX Share Pro", expanded=False):
    ...
    st.markdown(f'<a href="whatsapp://send?text=Hey! Schau dir mal diese geniale App an: {app_url}" style="display: block; width: 100%; padding: 10px; background-color: #25D366; color: white; text-align: center; text-decoration: none; border-radius: 5px; font-weight: bold;">🚀 App empfehlen (WhatsApp)</a>', unsafe_allow_html=True)
    ...
    try:
        ...
        if st.session_state.custom_text:
            ...
            if st.session_state.show_bg_custom_text:
                ...
                safe_rect(draw, [pos_x - tw_c//2 - 25, pos_y - th_c//2 - 15, pos_x + tw_c//2 + 25, pos_y + th_c//2 + 15], fill=(0,0,0,160), outline=st.session_state.c_custom_text, width=2)
            draw_text_with_shadow(draw, (pos_x, pos_y), st.session_state.custom_text, f_custom, fill=st.session_state.c_custom_text)
        ...
        if st.session_state.show_logo:
            if st.session_state.logo_type == "Smartes Logo": draw_graphical_logo(draw, (30, bh_t + 30), st.session_state.size_logo, st.session_state.c_line)
            else:
                p = get_logo_path()
                if p:
...
with st.expander("⚖️ Impressum & Datenschutz", expanded=False):
    st.markdown("**Impressum:** Jürgen Unterweger, Wangham 13, 4661 Roitham am Traunfall, Österreich. Kontakt: juergen.unterweger@outlook.at")
