...
if up_img and up_gpx:
    try:
        ...
        if pts:
            ...
            # Schrift laden (font.ttf bevorzugt)
            font_t = None
            ...
            # --- TEXTE SCHREIBEN ---
            # Oben: Tour Titel (Weiß)
            draw.text((w//2, bh_top//2), tour_title, fill="white", font=font_t, anchor="mm")
            
            # Unten: Datenzeile (Zahlen Weiß, Einheiten Blutrot)
            # Wir berechnen die Positionen manuell für den Farbwechsel
            full_stat_text = f"DIST {d_total:.1f} km   ELEV {int(a_gain)} m"
            # Um es einfach zu halten, zeichnen wir die Labels in Weiß
            ...
            st.download_button("🚀 BILD SPEICHERN", buf.getvalue(), "ride_pro_xxl.jpg", "image/jpeg")

    except Exception as e:
        st.error(f"Fehler: {e}")import streamlit as st
import gpxpy
from PIL import Image, ImageDraw, ImageFont
import io
import math
