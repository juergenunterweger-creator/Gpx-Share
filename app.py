...
# --- TRACKING & DATABASE SETUP (v2.8.2) ---
try:
...
def get_total_count():
    """Liest den aktuellen Stand aus dem Google Sheet."""
    if HAS_GSHEETS:
        try:
            conn = st.connection("gsheets", type=GSheetsConnection)
            # Wir cachen das Ergebnis für 1 Minute (60s), um die API zu schonen
            df = conn.read(worksheet="Stats", ttl=60)
...
def trigger_counter():
    """Erhöht den Zähler nach dem Download."""
...
def draw_graphical_logo(draw, pos, scale=1.0, color="#DA2323"):
    x, y = int(pos[0]), int(pos[1])
    icon_size = int(50 * scale); rgb = hex_to_rgba(color)
...
st.markdown("""
<style>
.header-box { text-align: center; background: linear-gradient(135deg, #111, #2a2a2a); padding: 20px; border-radius: 15px; box-shadow: 0px 10px 20px rgba(218, 35, 35, 0.4); margin-bottom: 30px; border: 1px solid #333; }
.header-title { font-size: 38px; font-weight: 900; background: linear-gradient(90deg, #ff4b4b, #da2323); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0; text-transform: uppercase; }
.counter-text { font-size: 20px; font-weight: bold; color: #da2323; margin-top: 10px; }
</style>
...
with cup1: up_gpx = st.file_uploader("📍 1. GPX Datei wählen", key="gpx_up")
with cup2: up_img = st.file_uploader("📸 2. Foto wählen", type=["jpg", "jpeg", "png"], key="img_up")

# --- OPTIONEN ---
with st.expander("⚙️ Einstellungen [v2.8.2]"):
    o1, o2 = st.columns(2)
    with o1:
...
with st.expander("ℹ️ Über GPX Share Pro", expanded=False):
    # COUNTER ANZEIGE
    total_stories = get_total_count()
    if total_stories is not None:
        st.markdown(f"### 🔥 Schon **{total_stories}** erstellte Storys")
        st.write("Werde Teil der Community und teile deine Tour!")
    else:
        st.info("Statistiken werden geladen...")
    
    st.markdown("---")
    st.markdown("**Copyright: Jürgen Unterweger**")
    st.markdown(f'<a href="https://www.paypal.com/donate?hosted_button_id=FF6FBUE84V7MG" target="_blank"><img src="https://www.paypalobjects.com/de_DE/i/btn/btn_donateCC_LG.gif" width="120"></a>', unsafe_allow_html=True)
    st.markdown(f'<a href="whatsapp://send?text=Schau%20dir%20diese%20App%20an!%20https://gpx-share-oh4dfakuqvfxadxmg3qhhq.streamlit.app/" style="display: block; width: 100%; padding: 10px; background-color: #25D366; color: white; text-align: center; text-decoration: none; border-radius: 5px; font-weight: bold; margin-top:10px;">🚀 App empfehlen</a>', unsafe_allow_html=True)

# --- APP INSTALLIEREN ---
with st.expander("📲 App installieren"):
    st.write("Tippe auf deinem Handy auf 'Zum Home-Bildschirm hinzufügen', um GPX Share Pro als App zu nutzen.")
...
if up_gpx:
    try:
        ...
        if st.download_button("🚀 BILD SPEICHERN", buf.getvalue(), "tour.png", "image/png"):
            trigger_counter()
            st.success("Tour gespeichert! Statistik aktualisiert.")
