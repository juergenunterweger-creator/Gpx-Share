...
st.markdown("""
    ...
    div[data-testid="stExpander"] details summary p {import streamlit as st
import gpxpy
from PIL import Image, ImageDraw, ImageFont, ImageChops
import io
import math
    ...
    try:
        ...
        if pts:
            ...
            if len(elevs) > 1:
                ...
                grid_text_color = (255, 255, 255, 160) 
                grid_y_start = h - bh_bot
                
                # Raster & Beschriftung Hm
                for i in range(1, 4):
            ...
            spacing, icon_gap, icon_size = int(w*0.12), int(w*0.02), int(auto_f_data * 1.3)
            ...
            draw.text((x_elev := start_x + icon_size + icon_gap + w_dist + spacing, y_pos), "", fill="white") # Helper
...
with st.expander("ℹ️ Über GPX Share", expanded=False):
    ...
    Lade einfach eine `.gpx` Datei hoch und setze deine Tour perfekt in Szene.
    """)
        ...
        if pts:
            ...
            if draw_line_manually:
                ...
                if len(scaled_pts) > 1:
                    ...
                    draw.ellipse([scaled_pts[0][0]-point_size, scaled_pts[0][1]-point_size, scaled_pts[0][0]+point_size, scaled_pts[0][1]+point_size], fill=(255,255,255, r_alpha))
                    draw.ellipse([scaled_pts[-1][0]-point_size, scaled_pts[-1][1]-point_size, scaled_pts[-1][0]+point_size, scaled_pts[-1][1]+point_size], fill=rgb + (r_alpha,))
            ...
            total_w = icon_size + int(w*0.02) + w_dist + int(w*0.12) + icon_size + int(w*0.02) + w_elev
            start_x = (w - total_w) // 2
            draw.text((start_x + icon_size + int(w*0.02), y_pos), txt_dist, fill="white", font=font_d, anchor="lm")
            draw.text((start_x + total_w - w_elev, y_pos), txt_elev, fill="white", font=font_d, anchor="lm")
            ...
            if show_logo and os.path.exists("logo.png"):
                ...
                if radius > 0:
                    ...
                    ImageDraw.Draw(mask).rounded_rectangle([0, 0, new_size[0], new_size[1]], fill=255, radius=radius)
