...
def draw_smooth_icon(mode, size, color="white"):
    ...
    if mode == "Sonne":
        ...
        d.rectangle([size*res*0.35, size*res*0.55, size*res*0.65, size*res*0.7], fill=color)
...
with st.expander("⚙️ Optionen & Design", expanded=False):
    ...
    with col_opt1:
        st.write("**🖼️ Hintergrund & Wetter**")
        st.slider("Hintergrund Sichtbarkeit (%)", 0, 100, key="bg_opacity")
        st.selectbox("Wetter-Icon", ["Sonne", "Wolken", "Regen", "Schnee"], key="weather")
    ...
    try:
        ...
        if segments_pts:
            ...
            if st.session_state.show_date and st.session_state.tour_date:
                ...
                bx1, by1 = bx2 - tw - 100, by2 - f_date_size - pad*2
                safe_rect(draw, [bx1, by1, bx2, by2], fill=rgb_box + (st.session_state.b_alpha,), outline="white")
                draw.text((bx2 - 20, by1 + pad), st.session_state.tour_date, fill="white", font=f_date)
